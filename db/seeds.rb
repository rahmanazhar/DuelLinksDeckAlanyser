...
pages[0..50].each{ |link|
    ...
    c = Card.find_or_initialize_by(name: card_page.css("div.article-part")[0].css("h2").text)
    ...
    stats.each{|s|
        ...
        case key.strip
            ...
            when "Attribute"
                c.update_attribute(:attribute_value , s.css("td")[1].text.to_s.downcase.capitalize)
                #puts s.css("td")[1].text.to_s
